...
func (s *Storage) CreateUser(user *model.User) (err error) {
	...
	query := `
		...
		VALUES
			(LOWER($1), $2, $3, $4)
		RETURNING
			id, username, is_admin, language, theme, timezone, entry_direction, entries_per_page, keyboard_shortcuts, show_reading_time, auto_mark_as_read, entry_embedded, left_menu_state
...
func (s *Storage) UpdateUser(user *model.User) error {
	if user.Password != "" {
		...
		_, err := s.db.Exec(
			...
			user.Language,
			user.Timezone,//$5
			user.EntryDirection,//$6
			user.EntriesPerPage,//$7
